...
| 3 | Schedule | ✅ Daily list, "where are we now" focus view + big checkmark (advances, calm celebration), read-the-day-aloud, monthly calendar of events, schedule editor (tasks w/ symbol + time, copy-day, events). Task completion goes through the offline outbox; `/api/schedule/day` cached network-first. Real-device airplane-mode test still pending. **← current** |
