...
| 6 | AI social stories | ✅ Caregiver chats with a five-slot interviewer agent → a writer → SLP-reviewer → illustrator crew composes a structured, reviewed story JSON. Compose saves the **reviewed text + read-aloud immediately**; illustrations fill in afterwards one page per request (`/illustrate`), so it never blocks on the image model. Child reads it in User Mode (page-turner). Gemini adapter written (four structured-output calls + a Gemini image model for pages; models env-configurable, image gen needs a billed key); **deterministic stub** runs the same shape without a key. LLM usage checked at compose, images metered per page. **Content guardrails:** a shared Hebrew content policy in all four role prompts + a reviewer-level hard refusal (`ContentRefused` → 422 `content_declined`, audited, nothing persisted, illustrator skipped) — protective body-safety education is explicitly allowed; caregiver-edited text (`PATCH /<id>`) is deliberately not gated. |
...
| 9 | Typing board | ✅ "הפתקים שלי" (לוח הקלדה) — a 7th module for a child who already types: free-composition notes with 3 block styles (כותרת ראשית / משנה / טקסט), a per-child typeface (Rubik / Assistant / Heebo) + size, offline draft (`kv`) + coalesced outbox upsert on a client-generated id, and a caregiver viewer (read / TTS on demand / share / print / delete). Migration `0023_typing_board.sql`. |
| 10 | My Tasks | ✅ "המשימות שלי" — an 8th module: a personal checklist the child ticks off (offline via the outbox), each task with a symbol and a `daily`/`once` recurrence (daily resets itself, one-off disappears the next day — one `completed_on` date, no sweep). Finishing every task due today releases a configurable token reward (0–20, default 1) once per local date, via a caregiver PIN gate (`kind: "tasks"` ledger entry). Caregiver editor: curate tasks + attach symbols + set the reward. Migration `0025_my_tasks.sql`. |
| 11 | Painting | ✅ "בוא נצייר" — a 9th module: paint on a blank white page, or colour inside a bundled Mulberry symbol rendered as line art (its colour shapes → white + tappable, its stroked outlines → black on top). Tools: 13-colour palette, brush (3 sizes), tap-to-fill bucket (colouring pages only), eraser, undo, "דף חדש". Stored as **vector JSON** — a list of normalised (0..1) brush/eraser strokes + region fills, capped (≤400 strokes / ≤60k total points / ≤300 fills); **no PNG server-side**, no Storage, no quota (a PNG is rasterised client-side only for share / download / print). Offline via a coalesced outbox upsert on a client-generated id, monotonic `rev`, `kv` draft. Colouring pages: a curated shortlist ships in `js/modules/painting/pages.js`; the caregiver adds more per child (symbol picker, with a runtime "is it colourable" check). Caregiver viewer: thumbnail gallery + share / download / print / delete. Migration `0027_painting.sql`; also ships batch 16 of the Mulberry set (268 symbols) so there are picture subjects to colour. No tokens — same stance as the typing board. |
