...
| 4 | Tokens & rewards | ✅ Behavior rule cards (visual + audio), token ledger (`token_transactions` source of truth + trigger-maintained balance), caregiver awards, reward store, redemption → held tokens → in-app approval queue + dashboard badge; reject refunds. **← current** |
